...
#include "SiYuanHeiTi_Bold_96.c"

LV_FONT_DECLARE(siyuanheiti_bold_96);
...
void welcome()
{
    ...
    lv_obj_set_style_text_font(label, &siyuanheiti_bold_96, LV_PART_MAIN | LV_STATE_DEFAULT);
    // lv_obj_add_style(label, &style, 0);                        // 将样式添加到文字对象中
    lv_label_set_text(label, "12：32：00"); // 显示文字
...
void setup()
{
    ...
    lv_obj_set_style_text_font(label, &SiYuanHeiTi_Bold_96, LV_PART_MAIN | LV_STATE_DEFAULT);
    lv_obj_add_style(label, &style, 0);                        // 将样式添加到文字对象中
    lv_label_set_text(label, "12：32：00!");
